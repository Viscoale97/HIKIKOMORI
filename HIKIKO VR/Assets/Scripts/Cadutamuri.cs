...
public class Cadutamuri : MonoBehaviour
{
    ...
    public void Crollare()
    {
        ...
        if (Muro_F)
        {
            rb.AddForce(1, -20, -3);
            rb.AddTorque(transform.right * Time.deltaTime);
            GameObject.Find("Porta").GetComponent<Rigidbody>().AddForce(1, -20, -3);
            GameObject.Find("Porta").transform.Rotate(2f * Time.deltaTime, 0.2f * Time.deltaTime, 0.2f * Time.deltaTime, Space.World);
